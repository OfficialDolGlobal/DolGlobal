...
import { token } from "../typechain-types/@openzeppelin/contracts";
  ...
  describe("Payment Pool", function () {
    ...
    await devPool.incrementBalance(ethers.parseUnits("10000",6),usdtAddress)
    await devPool.removeToken(dolGlobalAddress)
    await expect(devPool.incrementBalance(ethers.parseUnits("10000"),dolGlobalAddress)).to.be.revertedWith("Token contract not registered")
    await devPool.addToken(dolGlobalAddress,"DOL")
    await devPool.incrementBalance(ethers.parseUnits("10000"),dolGlobalAddress)
    ...
    await devPool.updateRecipientPercentage(another.address,300000)

    await devPool.updateRecipientPercentage(otherAccount.address,500000)

    await expect(devPool.updateRecipientPercentage(owner.address,100000)).to.be.revertedWith("This recipient is immutable and cannot have their percentage updated")
